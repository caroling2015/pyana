...
#查看总投资金额，分用户
def owner_invest():
    data = runSQL('select owner,sum(amount) from trade group by owner')
    return data
...
def check_fund_average(owner,fundid):
    ...
    data = runSQL ('select owner,fundid,sum(amount),sum(Quantity),min(price),round(sum(amount)/sum(Quantity),5) from Trade where Owner =? and FundID=?', (owner, fundid))
    return data
...
def tosell():
    ...
    for each in info:
        fundid,fundgz =each
        fundgz = float(fundgz)
        sql = 'select owner,fundid,round(sum(quantity),2) from trade where price <?-0.2  and fundid =? group by owner'
        data = runSQL(sql,(fundgz,fundid))
